      ...
      try {
        ...
        flowCard.registerRunListener(async (args, state) => {
          try {
            ...
            console.log(`${this.homey.manifest.id} - ${this.homey.manifest.version}: starting flow: ${flow.action} and sending command: `, flow.parsedCommand);

            return await SonyBraviaAndroidTvCommunicator.sendCommand(args.device, flow.parsedCommand);
          } catch (err) {
            console.log(`${this.homey.manifest.id} - ${this.homey.manifest.version}: flow command could not be executed: `, flow, err);
          }
        });
      } catch (err) {
        console.log(`${this.homey.manifest.id} - ${this.homey.manifest.version}: flow command could not be registered: `, flow, err);
      }
    });
  }
